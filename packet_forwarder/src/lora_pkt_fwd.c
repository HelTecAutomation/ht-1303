...
static char serv_addr1[64] = "127.0.0.2"; /* address of the server (host name or IPv4/IPv6) */
static char serv_port_up1[8] = "1701"; /* server port for upstream traffic */
static char serv_port_down1[8] = "1701"; /* server port for downstream traffic */
...
static int pull_cnt=0;

uint8_t extra_server=0;
static int sock_up1; /* socket for upstream traffic */
static int sock_down1; /* socket for downstream traffic */
...
static int parse_extraserver_configuration(const char * conf_file) {
    const char conf_obj_name[] = "server_conf";
    JSON_Value *root_val;
    JSON_Object *conf_obj = NULL;
    JSON_Value *val = NULL; /* needed to detect the absence of some fields */
    const char *str; /* pointer to sub-strings in the JSON data */
    ...
    str = json_object_get_string(conf_obj, "server_address");
    if (str != NULL) {
        strncpy(serv_addr1, str, sizeof serv_addr);
        serv_addr[sizeof serv_addr - 1] = '\0'; /* ensure string termination */
        MSG("INFO: server1 hostname or IP address is configured to \"%s\"\n", serv_addr1);
    }

    /* get up and down ports (optional) */
    val = json_object_get_value(conf_obj, "serv_port_up");
    if (val != NULL) {
        snprintf(serv_port_up1, sizeof serv_port_up1, "%u", (uint16_t)json_value_get_number(val));
        MSG("INFO: upstream1 port is configured to \"%s\"\n", serv_port_up1);
    }
    val = json_object_get_value(conf_obj, "serv_port_down");
    if (val != NULL) {
        snprintf(serv_port_down1, sizeof serv_port_down1, "%u", (uint16_t)json_value_get_number(val));
        MSG("INFO: downstream1 port is configured to \"%s\"\n", serv_port_down1);
    }
    val = json_object_get_value(conf_obj, "enable");
    if (val != NULL) {
        extra_server=(uint8_t)json_value_get_number(val);
        MSG("INFO: extra server enable is %d\n", extra_server);
    }
    /* free JSON parsing data structure */
    json_value_free(root_val);
    return 0;
...
static int send_tx_ack(uint8_t token_h, uint8_t token_l, enum jit_error_e error, int32_t error_value) {
    ...
    *(uint32_t *)(buff_ack + 8) = net_mac_l;
	...
	if(server_index==1)
	{
		return send(sock_down1, (void *)buff_ack, buff_index, 0);
	}
	else
	{
...
int main(int argc, char ** argv)
{
    ...
    struct addrinfo *result1; /* store result of getaddrinfo */
    struct addrinfo *q1; /* pointer to move into *result data */
    char host_name[64];
    char port_name[64];
    char host_name1[64];
    char port_name1[64];
    ...
    /* load configuration files */
    if (access("server_conf.json", R_OK) == 0) { /* if there is a global conf, parse it  */
        MSG("INFO: found configuration file %s, parsing it\n", conf_fname);
        x = parse_extraserver_configuration("server_conf.json");
    } else {
        MSG("ERROR: [main] failed to find any configuration file named %s\n", "server_conf.json");
    }
...
if(extra_server)
{
    ...
    hints1.ai_family = AF_INET; /* WA: Forcing IPv4 as AF_UNSPEC makes connection on localhost to fail */
    hints1.ai_socktype = SOCK_DGRAM;

    /* look for server address w/ upstream port */
    i = getaddrinfo(serv_addr1, serv_port_up1, &hints1, &result1);
    if (i != 0) {
        MSG("ERROR: [up] getaddrinfo on address %s (PORT %s) returned %s\n", serv_addr1, serv_port_up1, gai_strerror(i));
        exit(EXIT_FAILURE);
    }

    /* try to open socket for upstream traffic */
    for (q1=result; q1!=NULL; q1=q1->ai_next) {
        sock_up1 = socket(q1->ai_family, q1->ai_socktype,q1->ai_protocol);
        if (sock_up1 == -1) continue; /* try next field */
        else break; /* success, get out of loop */
    }
    if (q1 == NULL) {
        MSG("ERROR: [up] failed to open socket to any of server %s addresses (port %s)\n", serv_addr1, serv_port_up1);
        i = 1;
        for (q1=result; q1!=NULL; q1=q1->ai_next) {
            getnameinfo(q1->ai_addr, q1->ai_addrlen, host_name1, sizeof host_name1, port_name1, sizeof port_name1, NI_NUMERICHOST);
            MSG("INFO: [up] result %i host:%s service:%s\n", i, host_name1, port_name1);
            ++i;
        }
        exit(EXIT_FAILURE);
    }
    ...
    /* connect so we can send/receive packet with the server only */
    i = connect(sock_up1, q1->ai_addr, q1->ai_addrlen);
    if (i != 0) {
        MSG("ERROR: [up] connect returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    /* look for server address w/ downstream port */
    i = getaddrinfo(serv_addr1, serv_port_down1, &hints1, &result1);
    if (i != 0) {
        MSG("ERROR: [down] getaddrinfo on address %s (port %s) returned %s\n", serv_addr1, serv_port_down1, gai_strerror(i));
        exit(EXIT_FAILURE);
    }

    /* try to open socket for downstream traffic */
    for (q1=result; q1!=NULL; q1=q1->ai_next) {
        sock_down1 = socket(q1->ai_family, q1->ai_socktype,q1->ai_protocol);
        if (sock_down1 == -1) continue; /* try next field */
        else break; /* success, get out of loop */
    }
    if (q1 == NULL) {
        MSG("ERROR: [down] failed to open socket to any of server %s addresses (port %s)\n", serv_addr, serv_port_down);
        i = 1;
        for (q1=result1; q!=NULL; q1=q1->ai_next) {
            getnameinfo(q1->ai_addr, q1->ai_addrlen, host_name1, sizeof host_name1, port_name1, sizeof port_name1, NI_NUMERICHOST);
            MSG("INFO: [down] result %i host:%s service:%s\n", i, host_name1, port_name1);
            ++i;
        }
    ...
    /* connect so we can send/receive packet with the server only */
    i = connect(sock_down1, q1->ai_addr, q1->ai_addrlen);
    if (i != 0) {
        MSG("ERROR: [down] connect returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
...
if(extra_server)
{
    i1 = setsockopt(sock_up1, SOL_SOCKET, SO_RCVTIMEO, (void *)&push_timeout_half, sizeof push_timeout_half);
    if (i1 != 0) {
        MSG("ERROR: [up1] setsockopt returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        if(nb_pkt!=0)
        {
          FILE *fp=fopen("/tmp/frame_up.json","w");
          fprintf(fp,"%s",(char *)(buff_up + 12));
          fclose(fp);
        }
        /* send datagram to server */
...
if(extra_server)
{
        send(sock_up1, (void *)buff_up, buff_index, 0);
        clock_gettime(CLOCK_MONOTONIC, &send_time);
        pthread_mutex_lock(&mx_meas_up);
	wait_ms(FETCH_SLEEP_MS);
        /* wait for acknowledge (in 2 times, to catch extra packets) */
        for (i=0; i<2; ++i) {
            j = recv(sock_up1, (void *)buff_ack, sizeof buff_ack, 0);
            clock_gettime(CLOCK_MONOTONIC, &recv_time);
            if (j == -1) {
                if (errno == EAGAIN) { /* timeout */
                    continue;
                ...
                //MSG("WARNING: [up1] ignored invalid non-ACL packet\n");
                continue;
            } else if ((buff_ack[1] != token_h) || (buff_ack[2] != token_l)) {
                //MSG("WARNING: [up1] ignored out-of sync ACK packet\n");
                continue;
            } else {
                MSG("INFO: [up1] PUSH_ACK received in %i ms\n", (int)(1000 * difftimespec(recv_time, send_time)));
                break;
            }
        }
        pthread_mutex_unlock(&mx_meas_up);
...
if(extra_server)
{
    i = setsockopt(sock_down1, SOL_SOCKET, SO_RCVTIMEO, (void *)&pull_timeout, sizeof pull_timeout);
    if (i != 0) {
        MSG("ERROR: [down1] setsockopt returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
...
if(extra_server)
{
        send(sock_down1, (void *)buff_req, sizeof buff_req, 0);
}
        pull_cnt++;
        if( pull_cnt>=10)
        {
                MSG("INFO: [NET] server long time no response, exit\n");
                exit(EXIT_FAILURE);
        }
        clock_gettime(CLOCK_MONOTONIC, &send_time);
        pthread_mutex_lock(&mx_meas_dw);
...
if(extra_server)
{
            ...
            {  
			server_index=1;
		        msg_len = recv(sock_down1, (void *)buff_down, (sizeof buff_down)-1, 0);
            }
}
            clock_gettime(CLOCK_MONOTONIC, &recv_time);
          ...
          FILE *fp=fopen("/tmp/frame_down.json","w");
          fprintf(fp,"%s",(char *)(buff_down + 4));
          fclose(fp);
